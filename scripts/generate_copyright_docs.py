...
class CopyrightDocGenerator:
    ...
    def analyze_project(self, extra_excludes: Optional[List[str]] = None) -> Dict:
        self.project_info = {
            "name": "",
            "version": "",
            "description": "",
        ...
        self._collect_code_files(extra_excludes)
    ...
    def _build_matcher(self, patterns: List[str]):
        if not patterns:
            return None
        try:
            import pathspec
    ...
    def _is_excluded(self, relative_str: str, matcher) -> bool:
        if matcher is None:
            return False
        try:
            import pathspec
            if isinstance(matcher, pathspec.PathSpec):
                return matcher.match_file(relative_str)
        except ImportError:
            pass
        import fnmatch
        norm = relative_str.replace("\\", "/")
        parts = norm.split("/")
    ...
    def _collect_code_files(self, extra_excludes: Optional[List[str]] = None):
        exclude_dirs = [".git", "node_modules", "__pycache__", "dist", "build", ".trae"]

        gitignore_matcher = self._build_matcher(self._load_gitignore_patterns())
        extra_matcher = self._build_matcher(extra_excludes or [])
        ...
        for root, dirs, files in os.walk(self.project_path):
            ...
            for file in files:
                ...
                if self._is_excluded(relative_str, gitignore_matcher):
                    continue
                if self._is_excluded(relative_str, extra_matcher):
                    continue
    ...
    def generate_source_code_doc(
        ...
        code_files = [
            ...
            if not self._is_excluded(str(f).replace("\\", "/"), extra_matcher)
        ]
    ...
    def generate_application_form(
        self, output_path: str, owner_info: Optional[Dict] = None
    ):
        output_file = Path(output_path)
        output_file.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "示例:\n"
            "  python generate_copyright_docs.py /path/to/project\n"
            "  python generate_copyright_docs.py /path/to/project ./output --exclude tests docs '*.min.js'\n"
        ),
    )
    parser.add_argument("project_path", help="项目根目录路径")
    parser.add_argument("output_dir", nargs="?", help="输出目录（默认：<project_path>/copyright_docs）")
    parser.add_argument(
        "--exclude", "-e",
        nargs="*",
        metavar="PATTERN",
        default=[],
        help="额外排除的文件或目录（支持 glob 模式，如 tests docs '*.min.js'）",
    )
    args = parser.parse_args()

    project_path = args.project_path
    output_dir = args.output_dir or os.path.join(project_path, "copyright_docs")
    extra_excludes = args.exclude or []

    generator = CopyrightDocGenerator(project_path)

    print("正在分析项目...")
    project_info = generator.analyze_project(extra_excludes=extra_excludes if extra_excludes else None)
    ...
    print("\n正在生成源代码文档...")
    generator.generate_source_code_doc(
        os.path.join(output_dir, "源代码文档.md"),
        extra_excludes=extra_excludes if extra_excludes else None,
    )
    ...
    print("正在生成申请表...")
    generator.generate_application_form(
        os.path.join(output_dir, "软件著作权登记申请表.md")
    )
